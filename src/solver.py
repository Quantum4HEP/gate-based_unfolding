...
class Solver:
    ...
    def _sample_with_params(
        self, params: List[float], num_shots: int = 1000
    ) -> List[Dict[str, float]]:
        ...
        return [
            {"values": self._split_string_by_indices(key, self.qu_unfolder.num_bits), "counts": value}
    ...
    def _split_string_by_indices(self, s, indices) -> List[int]:
        ...
        for index in indices:
            parts.append(int(s[start:start + index], 2))
            start += index
        return parts
    ...
    def get_costs(self, params, cost_function: Callable, num_shots: int) -> float:
        ...
        counts = [result["counts"] for result in parsed_results]
        costs = [cost_function(result["values"]) for result in parsed_results]
    ...
    def _execute_circ(
        ...
        return final_params, self.cost_values
    ...
    def _get_gurobi_result(self):
        if self.gurobi_result is None:
            self.gurobi_result = self.qu_unfolder.solve_gurobi_integer()
        return self.gurobi_result

    def get_results_and_print(
        self, final_params, num_shots=1000, print_results=True
    ) -> List[int]:
    ...
    def get_p_t_value(self, best_results: List[int]):
        sol, cov = self._get_gurobi_result()
